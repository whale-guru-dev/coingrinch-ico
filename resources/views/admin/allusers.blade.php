								...
								<tbody>
									@foreach($duser as $user)
										<tr>
											...
											<th>{{$user->mobile}}</th>
											<th>
												@if(App\Models\Addresses::where('useremail',$user['email'])->first())
												<strong>BTC:</strong>
												{{App\Models\Addresses::where('useremail',$user['email'])->first()->btc}}
												<br><strong>ETH:</strong>
												{{App\Models\Addresses::where('useremail',$user['email'])->first()->eth}}
												@endif
											</th>
											<th>{{$user->acgc}}</th>
											<th><a href='{{url('/Admins/UserDetails/').'/'.$user->id}}' class='btn btn-success btn-md'><i class='fa fa-desktop'></i> VIEW DETAILS</a></th>
										</tr>
									@endforeach
								</tbody>
							</table>
